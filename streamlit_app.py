...
def generate_competitive_brief(company: str, use_news: bool, use_wikipedia: bool, num_articles: int) -> tuple:
    ...
    try:
        ...
        if "ResourceExhausted" in error_msg or "quota" in error_msg.lower():
            return f"API rate limit reached. Please wait 30-60 minutes. Data collection was successful ({len(news_items)} articles), but AI synthesis is temporarily unavailable.", processed
        else:
...
st.markdown("---")
st.caption("Marketing Intelligence Platform - AI-Powered Insights for Competitive Strategy")
